    ...
    <section class="content">
      ...
      <div class="box box-primary">
           ...
           <div class="box-header">
                  ...
                  <h4>Filter by Booking Status</h4>
                  <div class="divide10"></div>
                  <input class="orderFilter" type="radio" name="filter" value="-2" checked="checked"> &nbsp;All Bookings &nbsp; &nbsp;
                  <input class="orderFilter" type="radio" name="filter" value="0"> &nbsp;New Bookings &nbsp; &nbsp;
                  <!-- <input class="orderFilter" type="radio" name="filter" value="1"> &nbsp;Processed Bookings &nbsp; &nbsp;
                  <input class="orderFilter" type="radio" name="filter" value="2"> &nbsp;Dispatched Bookings &nbsp; &nbsp;
                  <input class="orderFilter" type="radio" name="filter" value="3"> &nbsp;Delivered Bookings &nbsp; &nbsp; -->
                  <input class="orderFilter" type="radio" name="filter" value="-1"> &nbsp;Cancelled Bookings
...
function search_order(){
    ...
    var x;
      if(arr.status=="success")
      {
          var out='<div class="box box-primary">';
              ...
              out+=' <tr>';
              // out+='<th>Sl.No</th>';
              out+='<th>Booking No.</th>';
              out+='<th>Customer Name </th>';
              // out+='<th>Company </th>';
              out+='<th>Booking Date </th>';
              out+='<th>Amount </th>';
              // out+='<th>Delivered Date</th>';
              out+='<th>Booking Status</th>';
      ...
      for(x=0;x<arr.orders.length;x++)
      {
          ...
          // out+="<td>"+(x+1)+"</td>";
             ...
             out+="<td><a href='../invoice/invoice.php?id="+arr.orders[x].orderNo+"' target='_blank'>Booking# "+arr.orders[x].orderNo+"</a></td>";
  ...
  //change order status "cancel to Process"
               if(arr.orders[x].order_status == "New Booking"){
                  out+="<button class='processOrder btn btn-info' id='"+arr.orders[x].orderNo+"'> Process Booking</button>";
                   }

  // //change order status "Process to Dispatch"
  //              else if(arr.orders[x].order_status == "Processed"){
  //                 out+="<button class='dispatchOrder btn btn-primary' id='"+arr.orders[x].orderNo+"'> Dispatch Booking</button>";
  //                  }
  // //change order status "Process to Dispatch"
  ...
  // //change order status "Dispatch to Delivered"
               ...
               if(arr.orders[x].order_status != "Cancelled" && arr.orders[x].order_status != "Delivered"){
                  out+=" <button class='cancelOrder btn btn-danger' id='"+arr.orders[x].orderNo+"'> Cancel Booking</button>";
                   }
