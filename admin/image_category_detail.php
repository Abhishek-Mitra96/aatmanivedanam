...
function print_data(data){
  ...
  if(arr.status=="success"){
    ...
    $("#category_name").text(arr.image_category[0].name);
    $("#category_description").text(arr.image_category[0].description);
    $("#edit_category_name").val(arr.image_category[0].name);
    $("#edit_category_description").val(arr.image_category[0].description);
    $("#edit_category_status[value='"+arr.image_category[0]["status"]+"']").attr('checked','checked');

    var i;
    var out='';

    var image_count=arr.image_list.length;
    if(image_count!=0){
      ...
      for(i=0;i<image_count;i++){
        ...
        out+='<td>';
        if(arr.image_list[i].status=="1")
          out+='<a href="image_detail.php?id='+arr.image_list[i].image_id+'" class="text-success">'+arr.image_list[i].name+'</a>';
        else out+='<a href="image_detail.php?id='+arr.image_list[i].image_id+'" class="text-danger">'+arr.image_list[i].name+'</a>';
        out+='</td>';

        out+='<td>'+arr.image_list[i].description+'</td>';

        out+='<td><div class="row">';
        if(arr.image_list[i].status=="1"){
          //category is in active mode. press this to deactivate
          out+='<a class="btn btn-danger btn-block" onclick="toggleStatus('+arr.image_list[i].image_id+')" data-toggle="tooltip" title="click for deactivate"><i class="fa fa-times"></i>&nbsp;Active</a>';
        }else{
          //category is in inactive mode. press this to activate
          out+='<a class="btn btn-success btn-block" onclick="toggleStatus('+arr.image_list[i].image_id+')" data-toggle="tooltip" title="click for activate"><i class="fa fa-check"></i>&nbsp;Inactive</a>';
        }
...
function toggleStatus(image_id){
  swal({
    title: "Are you sure ?",
    type: "warning",
  ...
  function(){
    $.ajax({
            type: 'POST',
            url: '../adminapi/image/image_change_status.php',
            data: {
                  image_id:image_id
                },
            success: function(data) {
...
function edit(){
  ...
  if(category_id!=""){
    $.ajax({
        type: 'POST',
        url: '../adminapi/image/category_edit.php',
        data: {
