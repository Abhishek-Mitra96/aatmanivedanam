...
$audio_category=json_decode(audioCategoryList($obj));
?>
</section>
  ...
  <section class="content">
    <div class="box box-primary" id="box-widget">
      ...
      <div class="box-body">
          ...
          <div class="col-sm-8">
            ...
            <div class="form-group">
              <select class="form-control chosen-select" id="category_list" name="category_list[]" tabindex="3">
                  <?php
                      for($i=0;$i<sizeof($audio_category->audio_category);$i++){
                          echo '<option value="'.$audio_category->audio_category[$i]->category_id.'">'.$audio_category->audio_category[$i]->name.'</option>';
                      }
                  ?>
              </select>
            </div>
          ...
          <div class="col-xs-8">
            <table class="table table-striped">
              <tr>
                ...
                <td><input type="text" name='add_audio_name' id="add_audio_name"  placeholder='Audio Name' class="form-control"/></td>
              </tr>
              <tr>
                <td><b>Discription</b></td>
                <td><textarea name="add_audio_description" id="add_audio_description" placeholder="Description" rows="3" class="form-control"></textarea></td>
              </tr>
              <tr>
                <td><b>Audio File</b></td>
                <td><input type="file" name='add_audio_file' id="add_audio_file" class="form-control"/></td>
              </tr>
            </table>
          </div>
          <div class="col-xs-2"></div>
        ...
        <div class="row">
          <center><button class="btn btn-primary" onclick="add_audio()">Add Image</button></center>
        </div>
      </div>
...
function add_category(){
  ...
  if(name!="" && description!=""){
    $.ajax({
        type: 'POST',
        url: '../adminapi/audio/category_add.php',
        data: {
              name:name,
...
function add_audio(){
  ...
  name=$("#add_audio_name").val();
  file=$("#add_audio_file").val(); 
  description=$("#add_audio_description").val();
  category_id=$("#category_list").val();
  //console.log(file);

  if(name!="" && description!="" && file!="" && category_id!=""){
    if(validate()){
      var filedata = $('#add_audio_file').prop('files')[0];
      ...
      $.ajax({
            type: 'post',
            url : '../adminapi/audio/audio_add.php',
            cache: false,
            contentType: false,
            processData: false,
            data  :  formdata,
...
function validate()
{
  ...
  var files = $('#add_audio_file').get(0).files;
  ...
  for (i = 0; i < files.length; i++){
    ...
    if (arrayExtensions.lastIndexOf(ext) == -1) {
      toast("Only mp3 audio allowed");
      return false;
    }
    //check for file size
    if(files[i].size > 307200)
    {
    toast("All audio should be within 300KB");
    return false;
    }
  }
  return true;
