...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <h1>  Manage Blog Category
             
              <a href="blog_category_insert.php" class="btn btn-info pull-right" role="button">Add Blog Category</a>
             </h1>
    </section>
...
function search_category(){
    ...
    var status=$('input:radio[name=filter]:checked').val();
        ...
        $.ajax({
            type: 'POST',
            url: '../adminapi/blog_category/blog_category_list.php',
            data: {
                search:searchkey,
                page:page,
                <?php
    ...
    for(x=0;x<arr.categories.length;x++)
        {
            ...
            out+="<tr>";
            out+="<td>"+(x+1)+"</td>";
            out+="<td><a href='category.php?parent="+arr.categories[x].blog_cat_id+"'>"+arr.categories[x].category_name+"</td>";
           // out+="<td>"+arr.categories[x].position+"</td>";
            ...
            out+="<td><button class='btn btn-danger toggleBlogCategoryStatus' cid='"+arr.categories[x].blog_cat_id+"'>";
            out+="<i class=\"fa fa-times\" aria-hidden=\"true\"></i>&nbsp Deactivate</button>"; 
        }
        else{
            out += "<td class='danger-p1'>" + status + "</td>";
            out+="<td><button class='btn btn-info toggleBlogCategoryStatus' cid='"+arr.categories[x].blog_cat_id+"'>";
            out+="<i class=\"fa fa-check\" aria-hidden=\"true\"></i>&nbsp Activate </button>"; 
     }

            out+="&nbsp;&nbsp;<a href='blog_category_edit.php?id="+arr.categories[x].blog_cat_id+"'class='btn btn-success' data-toggle='tooltip' title='Edit Category'><i class='fa fa-pencil-square-o'></i> Edit</a>";
