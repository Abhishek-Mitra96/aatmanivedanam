...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> Videos Category
    <a href="video_add.php" class="btn btn-info pull-right" role="button">Add Category</a>
  </section>
...
function print_data(data){
  ...
  if(arr.status=="success"){
    ...
    var category_count=arr.video_category.length;
    if(category_count!=0){
      ...
      for(i=0;i<category_count;i++){
        ...
        out+='<td>';
        if(arr.video_category[i].status=="1")
          out+='<a href="video_category_detail.php?id='+arr.video_category[i].category_id+'" class="text-success">'+arr.video_category[i].name+'</a>';
        else out+='<a href="video_category_detail.php?id='+arr.video_category[i].category_id+'" class="text-danger">'+arr.video_category[i].name+'</a>';
        out+='</td>';

        out+='<td>'+arr.video_category[i].description+'</td>';

        out+='<td><div class="row">';
        if(arr.video_category[i].status=="1"){
          //category is in active mode. press this to deactivate
          out+='<a class="btn btn-danger btn-block" onclick="toggleStatus('+arr.video_category[i].category_id+')" data-toggle="tooltip" title="click for deactivate"><i class="fa fa-times"></i>&nbsp;Active</a>';
        }else{
          //category is in inactive mode. press this to activate
          out+='<a class="btn btn-success btn-block" onclick="toggleStatus('+arr.video_category[i].category_id+')" data-toggle="tooltip" title="click for activate"><i class="fa fa-check"></i>&nbsp;Inactive</a>';
        }
...
function toggleStatus(category_id){
  ...
  function(){
    $.ajax({
            type: 'POST',
            url: '../adminapi/video/category_change_status.php',
            data: {
