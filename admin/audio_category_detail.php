...
function search(){
    var searchkey = $('#search').val();
       ...
       var page=$(".active1").attr("page");
        ...
        $.ajax({
            type: 'POST',
            url: '../adminapi/audio/category_list.php',
            data: {
...
function print_data(data){
  ...
  if(arr.status=="success"){
    ...
    $("#category_name").text(arr.audio_category[0].name);
    $("#category_description").text(arr.audio_category[0].description);
    $("#edit_category_name").val(arr.audio_category[0].name);
    $("#edit_category_description").val(arr.audio_category[0].description);
    $("#edit_category_status[value='"+arr.audio_category[0]["status"]+"']").attr('checked','checked');

    var i;
    var out='';

    var audio_count=arr.audio_list.length;
    if(audio_count!=0){
      ...
      out+='<tr>';
      // out+='<th class="col-xs-1">Image</th>';
      out+='<th class="col-xs-4">Name</th>';
      out+='<th class="col-xs-7">Description</th>';
      out+='<th class="col-xs-1">Action</th>';
      out+='</tr>';
      for(i=0;i<audio_count;i++){
        out+='<tr>';

        //out+='<td><div class="thumbnail"><img src="'+arr.dir_url+''+arr.image_list[i].url+'" class="img-rounded img-responsive" alt="image" width="100%"></div></td>';

        out+='<td>';
        if(arr.audio_list[i].status=="1")
          out+='<a href="audio_detail.php?id='+arr.audio_list[i].audio_id+'" class="text-success">'+arr.audio_list[i].name+'</a>';
        else out+='<a href="audio_detail.php?id='+arr.audio_list[i].audio_id+'" class="text-danger">'+arr.audio_list[i].name+'</a>';
        out+='</td>';

        out+='<td>'+arr.audio_list[i].description+'</td>';

        out+='<td><div class="row">';
        if(arr.audio_list[i].status=="1"){
          //category is in active mode. press this to deactivate
          out+='<a class="btn btn-danger btn-block" onclick="toggleStatus('+arr.audio_list[i].audio_id+')" data-toggle="tooltip" title="click for deactivate"><i class="fa fa-times"></i>&nbsp;Active</a>';
        }else{
          //category is in inactive mode. press this to activate
          out+='<a class="btn btn-success btn-block" onclick="toggleStatus('+arr.audio_list[i].audio_id+')" data-toggle="tooltip" title="click for activate"><i class="fa fa-check"></i>&nbsp;Inactive</a>';
        }
...
function toggleStatus(audio_id){
  swal({
    title: "Are you sure ?",
    type: "warning",
  ...
  function(){
    $.ajax({
            type: 'POST',
            url: '../adminapi/audio/audio_change_status.php',
            data: {
                  audio_id:audio_id
                },
            success: function(data) {
...
function edit(){
  ...
  if(category_id!=""){
    $.ajax({
        type: 'POST',
        url: '../adminapi/audio/category_edit.php',
        data: {
