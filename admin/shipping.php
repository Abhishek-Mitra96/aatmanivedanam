...
function search_product(){
    var searchkey = $('#search_product').val();
        ...
        $.ajax({
            type: 'POST',
            url: '../adminapi/shipping/shipping_search.php',
            data: {
                key:searchkey
                },
 ...
 function print_data(fetchdata)
    ...
    var out='<div class="box box-primary"><div class="box-body table-responsive no-padding">';
            ...
            out+='<th>Id </th>'
            out+='<th>Order No.</th>';
            out+='<th>Courier Name </th>';
            out+='<th>Date </th>';
            out+='<th>Date of delivery</th>';
    ...
    for(x=0;x<arr.length;x++)
    {
           ...
           /*****************************************pagination total_row(valu) end**************************************/
            ...
            out+="<tr>";
            out+="<td>"+arr[x].id+"</td>";
            out+="<td>"+arr[x].order_id+"</td>";
            out+="<td>"+arr[x].courier_name+"</td>";
            out+="<td>"+arr[x].date+"</td>";
            out+="<td>"+arr[x].date_of_delivery+"</td>";
            
           
     //    if(arr[x].visibility=="Active"){
     //        out += "<td class='success-p1'>" + arr[x].visibility + "</td>";
     //        out+="<td><a class='btn btn-danger' ";
     //        out+="href='../adminapi/shipping/change_shipping_status.php?id="+arr[x].id+"'  onclick=\"return confirm('Are you sure you want to change status "+arr[x].visibility+"?');\"><i class=\"fa fa-times\" aria-hidden=\"true\"></i> Deactivate</a> "; 
     //    }
     //    else{
     //        out += "<td class='danger-p1'>" + arr[x].visibility + "</td>";
     //        out+="<td><a class='btn btn-info' ";
     //        out+="href='../adminapi/shipping/change_shipping_status.php?id="+arr[x].id+"'  onclick=\"return confirm('Are you sure you want to change status "+arr[x].visibility+"?');\"><i class=\"fa fa-check\" aria-hidden=\"true\"></i> Activate </a> &nbsp;"; 
     // }
            out+="<td><a href='shipping_edit.php?id="+arr[x].id+"'class='btn btn-success' data-toggle='tooltip' title='Edit Shipment' data-confirm='#'><i class='fa fa-pencil-square-o'></i>Edit</a>";
            out+="  &nbsp;<a href='../adminapi/shipping/shipping_delete.php?id="+arr[x].id+"' class='btn btn-danger' onclick=\"return confirm('Are you sure you want to delete this item?');\" data-toggle='tooltip' data-confirm='#''><i class='fa fa-trash'></i> Delete</a>";
            out+="</td>";
            out+="</tr>";
    }
            out+="</tbody></table></div>";
 ...
 /*****************************************pagination tab for forntend end**************************************/       
    ...
    /******************pagination function for forntend end******************/
     function pagination(var1){
         ...
         var result_per_page = 5;
        $.post( "../adminapi/shipping/shipping_search.php", { pageno: var1, paginationCount: result_per_page})
