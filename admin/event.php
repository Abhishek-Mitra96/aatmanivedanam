...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> Events
    <a href="event_add.php" class="btn btn-info pull-right" role="button">Add Events</a>
  </section>
...
function search(){
    var searchkey = $('#search').val();
       ...
       var page=$(".active1").attr("page");
        ...
        $.ajax({
            type: 'POST',
            url: '../adminapi/event/event_list.php',
            data: {
                search:searchkey,
                status:status,
                page:page,
...
function print_data(data){
  ...
  if(arr.status=="success"){
    ...
    var event_count=arr.event_list.length;
    if(event_count!=0){
      ...
      for(i=0;i<event_count;i++){
        ...
        out+='<td>';
        if(arr.event_list[i].status=="1")
          out+='<a href="event_detail.php?id='+arr.event_list[i].event_id+'" class="text-success">'+arr.event_list[i].name+'</a>';
        else out+='<a href="event_detail.php?id='+arr.event_list[i].event_id+'" class="text-danger">'+arr.event_list[i].name+'</a>';
        out+='</td>';

        out+='<td>'+arr.event_list[i].description+'</td>';

        if(arr.event_list[i].datetime=="0000-00-00 00:00:00"){
          out+='<td>NA</td>';
        }else{
          out+='<td>'+arr.event_list[i].date_time+'</td>';
        }

        out+='<td><div class="row">';
        if(arr.event_list[i].status=="1"){
          //category is in active mode. press this to deactivate
          out+='<a class="btn btn-danger btn-block" onclick="toggleStatus('+arr.event_list[i].event_id+')" data-toggle="tooltip" title="click for deactivate"><i class="fa fa-times"></i>&nbsp;Active</a>';
        }else{
          //category is in inactive mode. press this to activate
          out+='<a class="btn btn-success btn-block" onclick="toggleStatus('+arr.event_list[i].event_id+')" data-toggle="tooltip" title="click for activate"><i class="fa fa-check"></i>&nbsp;Inactive</a>';
        }
...
function toggleStatus(event_id){
  swal({
    title: "Are you sure ?",
    type: "warning",
  ...
  function(){
    $.ajax({
            type: 'POST',
            url: '../adminapi/event/event_change_status.php',
            data: {
                  event_id:event_id
                },
            success: function(data) {
